...
parameter_file <- "sim_params/fig17_full_oracle_params.csv"
line_number <- 1
...
for(row in 1:nrow(results)) {
  ...
  for(b in 1:B) {
    ...
    eval_true_D0 <- dbeta(x = Y_0, shape1 = alpha_param, shape2 = beta_param)
    
    # Get log-concave MLE on D_0
    log_concave_D0 <- logConDens(x = Y_0, smoothed = FALSE)

    # Evaluate log-concave MLE on D_0
    eval_log_concave_D0 <- evaluateLogConDens(xs = Y_0, res = log_concave_D0)[, 3]
...
fwrite(results, 
       file = paste0("data/fig17_full_oracle_", line_number, ".csv"))
