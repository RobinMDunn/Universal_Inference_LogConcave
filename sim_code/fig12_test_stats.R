...
results <- data.table(n_obs = n_obs, d = d, mu_norm = mu_norm, B = B,
                      sim_id = sim_id,
...
results[, orig_test_stat := orig_ts]
...
for(b in 1:B) {
  ...
  results[b, shuffle_test_stat := shuffle_ts[b]]

}

# Reject H_0 if orig_ts > (B+1)*(1-alpha) quantile of shuffle_ts
results[, reject := as.numeric(orig_ts > sort(shuffle_ts)[ceiling((B+1)*(1-alpha))])]

# Save simulation results
fwrite(results, file = paste0("data/fig12_perm_test_stats_", line_number,
                              ".csv"))