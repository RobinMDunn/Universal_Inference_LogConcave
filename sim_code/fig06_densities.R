...
# Less than 1 second to run log-concave MLE functions on d = 1, n = 50.
...
density_df[mu_norm == 0 & n_obs == 50 & d == 1, sim := 7]
density_df[mu_norm == 2 & n_obs == 50 & d == 1, sim := 8]
density_df[mu_norm == 4 & n_obs == 50 & d == 1, sim := 9]

# Data frame to store simulated points
density_points <- 
  data.table(mu_norm = c(rep(c("||u|| = 0", "||u|| = 2", "||u|| = 4"),
                             each = 50)),
             sim = rep(c(7, 8, 9), each = 50),
             x = NA_real_)
...
for(sim_val in 7:9) {
  ...
  mean_loglik_logcondens_eval <- mean(evaluateLogConDens(
...
fwrite(density_df, file = "data/fig06_densities.csv")

# Save simulation points
fwrite(density_points, file = "data/fig06_points.csv")
