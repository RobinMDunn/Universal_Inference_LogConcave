...
# Read in data
perm_test <- fread("data/fig17_perm_test.csv") %>% 
  mutate(Method = "Permutation test (B = 99 shuffles)")

full_oracle <- fread("data/fig17_full_oracle.csv") %>% 
  mutate(Method = "Full oracle (B = 100)")

partial_oracle <- fread("data/fig17_partial_oracle.csv") %>% 
  mutate(Method = "Partial oracle (B = 100)")

fully_NP <- fread("data/fig17_fully_NP.csv") %>% 
