...
# Read in data
perm_test <- fread("data/fig04_perm_test.csv") %>% 
  mutate(Method = "Permutation test")

full_oracle_ddim <- fread("data/fig04_full_oracle_ddim.csv") %>% 
  mutate(Method = "Full oracle, d-dim")

partial_oracle_ddim <- fread("data/fig04_partial_oracle_ddim.csv") %>% 
  mutate(Method = "Partial oracle, d-dim")

partial_oracle_axis <- fread("data/fig04_partial_oracle_axis.csv") %>% 
  mutate(Method = "Partial oracle, axis-aligned projections")

fully_NP_axis <- fread("data/fig04_fully_NP_axis.csv") %>% 
  mutate(Method = "Fully nonparametric, axis-aligned projections")

partial_oracle_randproj <- fread("data/fig04_partial_oracle_randproj.csv") %>% 
  mutate(Method = "Partial oracle, random projections")

fully_NP_randproj <- fread("data/fig04_fully_NP_randproj.csv") %>% 
