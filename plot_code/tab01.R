...
# Read in data
perm_test <- fread("data/tab01_perm_test.csv")

full_oracle_ddim <- fread("data/tab01_full_oracle_ddim.csv")

partial_oracle_ddim <- fread("data/tab01_partial_oracle_ddim.csv")

partial_oracle_axis <- fread("data/tab01_partial_oracle_axis.csv")

fully_NP_axis <- fread("data/tab01_fully_NP_axis.csv")

partial_oracle_randproj <- fread("data/tab01_partial_oracle_randproj.csv")

fully_NP_randproj <- fread("data/tab01_fully_NP_randproj.csv")
...
all_data <- all_data %>% 
  mutate(Method = factor(Method, 
                         ...
                         labels = c("Partial oracle, random projections",
                                    "Fully NP, random projections",
                                    "Permutation test",
                                    "Fully NP, axis projections",
                                    "Partial oracle, axis projections",
                                    "Partial oracle, d-dim",
                                    "Full oracle, d-dim")))
