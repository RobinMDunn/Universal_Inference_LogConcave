...
# Read in data
perm_test <- fread("data/fig05_perm_test.csv") %>% 
  mutate(Method = "Permutation test")

full_oracle_ddim <- fread("data/fig05_full_oracle_ddim.csv") %>% 
  mutate(Method = "Full oracle, d-dim")

partial_oracle_ddim <- fread("data/fig05_partial_oracle_ddim.csv") %>% 
  mutate(Method = "Partial oracle, d-dim")

partial_oracle_axis <- fread("data/fig05_partial_oracle_axis.csv") %>% 
  mutate(Method = "Partial oracle, axis-aligned projections")

fully_NP_axis <- fread("data/fig05_fully_NP_axis.csv") %>% 
  mutate(Method = "Fully nonparametric, axis-aligned projections")

partial_oracle_randproj <- fread("data/fig05_partial_oracle_randproj.csv") %>% 
  mutate(Method = "Partial oracle, random projections")

fully_NP_randproj <- fread("data/fig05_fully_NP_randproj.csv") %>% 
...
reject_df <- results %>% 
  group_by(n_obs, d, mu_norm, Method, equal_space_mu) %>% 
...
stopifnot(unique(perm_test$B) == 99,
          ...
          unique(results$equal_space_mu) == 1,
...
# Plot rejection proportions at (||mu||*d^{-1/2}, ..., ||mu||*d^{-1/2})
reject_props_equal_spaced <- reject_df %>% 
...
# Plot rejection proportions at (||mu||*d^{-1/2}, ..., ||mu||*d^{-1/2}) 
# for single dim approaches, zooming in further on ||mu|| \in [4, 12]
reject_props_equal_zoom_onerow <- reject_df %>% 
  filter(mu_norm >= 4, mu_norm <= 12,
...
ggsave(plot = reject_props_equal_spaced,
       filename = "plots/figure_05a.pdf",
       width = 13, height = 8)

ggsave(plot = reject_props_equal_zoom_onerow,
       filename = "plots/figure_05b.pdf",
       width = 12, height = 3)
